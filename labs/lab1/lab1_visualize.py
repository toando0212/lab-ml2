...
def main():
    print("=== BREAST CANCER DATASET: PCA VISUALIZATION (Elbow, Kaiser, 2D) ===\n")
    ...
    # 2. Slice to first 10 columns (id, diagnosis, + 8 features)
    df = df.iloc[:, :10]
    ...
    print(f"Features: {len(feature_names)} (First 10 dimensions)")
    ...
    # Highlight the "elbow" point visually
    plt.title('Scree Plot & Kaiser\'s Rule (Breast Cancer - 8 Features)', fontsize=14, fontweight='bold')
    ...
    plt.axvline(x=n_components_95, color='orange', linestyle=':', linewidth=2, 
                label=f'95% at {n_components_95} Components')

    plt.title('Cumulative Explained Variance (Breast Cancer)', fontsize=14, fontweight='bold')
    ...
    plt.title('2D PCA Projection (Breast Cancer)', fontsize=15, fontweight='bold')
    ...
    print(f"  - Dimension reduction: 8 → {n_components_95}")
    print(f"  - Total variance captured: {cumulative_variance[n_components_95-1]:.1%}")
