...
def main():
    ...
    # Slice to first 10 columns (id, diagnosis, + 8 features)
    df = df.iloc[:, :10]
    
    # 2. Preprocess Data
    # Drop non-feature columns
    # 'id' is identifier, 'diagnosis' is label. Unnamed: 32 is not in first 10 cols.
    features_df = df.drop(columns=['id', 'diagnosis'], errors='ignore')
    
    print(f"Feature Scope: First 10 Dimensions (8 Numeric Features)")
    print(f"Features: {list(features_df.columns)}")
    print(f"Shape: {features_df.shape}")
    
    # 3. Standardization (Critical for PCA)
    # PCA is sensitive to scale. We must transform data to mean=0, std=1
    scaler = StandardScaler()
    scaled_data = scaler.fit_transform(features_df)
    
    # 4. Apply PCA
    # We don't restrict n_components yet, we want to see the full spectrum
    pca = PCA()
    pca.fit(scaled_data)
