...
The methodology implemented in this research is structured as an multi-phased experimental pipeline, designed to systematically evaluate the efficacy of deep feature representations for traffic sign classification. By bridging the gap between high-capacity convolutional neural networks (CNNs) and robust Support Vector Machines (SVMs), we investigate how different latent space geometries affect decision boundary complexity and overall model performance. The end-to-end flow of this pipeline, from initial data selection to final diagnostic evaluation, is visualized in Figure \ref{fig:pipeline_flow}.
...
The selected 10-class subset comprises a total of \textbf{14,670} photographic samples. As illustrated in Figure \ref{fig:class_dist}, the sample distribution across these 10 classes remains relatively balanced, with most categories containing between 1,400 and 2,200 images (with the exception of Class 0 and 6, which are naturally less frequent in the benchmark), ensuring that the model training process is not heavily biased toward any particular category.
...
\begin{enumerate}
    ...
    \item \textbf{Adjusted Rand Index ($ARI$):} A supervised metric that measures the similarity between two assignments (K-Means clusters vs. ground truth). It accounts for chance, where $+1$ is a perfect match and $\approx 0$ is random labeling.
    
    \item \textbf{Normalized Mutual Information ($NMI$):} Quantifies the amount of information shared between the predicted clusters and ground truth labels, normalized to range from $0$ to $1$.
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccc}
        \toprule
        \textbf{Architecture} & \textbf{Silhouette ($\uparrow$)} & \textbf{DB ($\downarrow$)} & \textbf{ARI ($\uparrow$)} & \textbf{NMI ($\uparrow$)} \\
        \midrule
        ResNet50 & -0.0090 & 6.6570 & 0.0504 & 0.1163 \\
        InceptionV3 & -0.0109 & 6.7656 & 0.0541 & 0.1098 \\
        \bottomrule
    \end{tabular}
\end{table}

The emergence of negative Silhouette scores and near-zero ARI/NMI values across both architectures requires a rigorous technical interpretation. Rather than indicating poor feature quality, we argue that these results represent a \textit{Geometric-Topological Paradox} inherent in high-dimensional deep representations. 

This phenomenon stems from the fundamental divergence between Euclidean distance metrics and non-Euclidean manifold geometry. Silhouette and ARI calculation in this stage rely on Euclidean proximity; however, traffic sign features typically reside on low-dimensional manifolds with high curvature. In such spaces, semantic neighbors (same class) may be Euclidean-distant, while unrelated samples from different classes appear Euclidean-proximal, leading to massive numerical overlap. The low ARI scores ($\approx 0.05$) specifically prove that traditional distance-based algorithms like K-Means fail to recover the latent class structure in the 2048-D space.

Ultimately, these "poor" metrics provide a vital methodological justification. They serve as empirical evidence that the feature space is not linearly or Euclidean-separable, thereby explicitly necessitating the adoption of non-linear "unfolding" strategies, such as \textit{Manifold Learning (UMAP)} and \textit{Radial Basis Function (RBF)} kernels, to resolve class boundaries that remain invisible to linear hyperplanes.
