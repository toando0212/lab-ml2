...
To rigorously evaluate the quality of the extracted features, we perform a comparative analysis between ResNet50 and InceptionV3. This evaluation employs a dual-framework approach:
\begin{itemize}
    \item \textbf{Linear Projection (PCA):} Assesses the global structural information and information density.
    \item \textbf{Non-linear Manifold Learning (UMAP):} Evaluates the local topological structure of the 10-class traffic sign clusters in high-dimensional space.
\end{itemize}
The analysis is implemented using \texttt{Scikit-learn}, \texttt{umap-learn}, and \texttt{Matplotlib}.

\subsection{Evaluation Criteria}
The assessment of feature quality is governed by two primary mathematical indices and a topological analysis:
...
\begin{enumerate}
    ...
    \end{equation}
    where $a(i)$ is the mean intra-cluster distance and $b(i)$ is the mean nearest-cluster distance. A value closer to +1 indicates high-quality clustering.
...
\begin{figure}[H]
    \centering
    \begin{tabular}{cc}
        % MOCKUP FOR 4 FIGURES (PCA 3D vs UMAP 3D)
        \framebox[0.45\textwidth]{\rule{0pt}{100pt} ResNet50: PCA 3D Visualization} & 
        \framebox[0.45\textwidth]{\rule{0pt}{100pt} InceptionV3: PCA 3D Visualization} \\
        (a) Linear Projection (PCA) & (b) Linear Projection (PCA) \\[10pt]
        \framebox[0.45\textwidth]{\rule{0pt}{100pt} ResNet50: UMAP 3D Visualization} & 
        \framebox[0.45\textwidth]{\rule{0pt}{100pt} InceptionV3: UMAP 3D Visualization} \\
        (c) Non-linear Manifold (UMAP) & (d) Non-linear Manifold (UMAP) \\
    \end{tabular}
    \caption{Qualitative comparison of feature manifolds. (Mockup for 3D visualization results).}
    \label{fig:3d_mockup}
\end{figure}
