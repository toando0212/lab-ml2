...
\titlegraphic{\hfill\includegraphics[height=1.5cm]{usth.png}}
...
\begin{frame}{Pipeline Architecture}
    ...
    \end{center}
    \begin{enumerate}
        \item \textbf{Extraction:} ResNet50 \& InceptionV3 backbones.
        \item \textbf{Dimensionality Reduction:} PCA \& UMAP (3D).
        \item \textbf{Diagnostic Strategy:} Classification on 3D manifolds.
    \end{enumerate}
\end{frame}
...
\begin{frame}{Qualitative Manifold Assessment}
    \begin{table}
        \centering
        \small
        \begin{tabularx}{\textwidth}{L C C}
            \toprule
            \textbf{Metric} & \textbf{PCA (Linear)} & \textbf{UMAP (Non-linear)} \\
            \midrule
            Silhouette Score ($\uparrow$) & -0.139 & \textbf{-0.060} \\
            Davies-Bouldin Index ($\downarrow$) & 11.126 & \textbf{6.022} \\
            \bottomrule
        \end{tabularx}
        \caption*{3D Manifold Separability Comparison}
    \end{table}
    \begin{itemize}
        \item \textbf{UMAP Advantage:} ~45\% improvement in DBI.
        \item \textbf{Non-linearity:} UMAP captures complex structures that PCA fails to isolate.
    \end{itemize}
\end{frame}
...
\begin{frame}{Summary}
  \begin{itemize}
    ...
    \item \textbf{RBF kernels} are essential for 2048-D features.
    \item 3D projections retain ~82\% accuracy, proving UMAP's density preservation.
  \end{itemize}
  \vspace{0.4cm}
  \begin{center}
